...
public class VoisePlayingIconView extends View {
    ...
    //指针波动速率
    public int pointerSpeed = 40;


    public VoisePlayingIconView(Context context) {
        this(context, null);
    }

    public VoisePlayingIconView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);

    }

    public VoisePlayingIconView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        //取出自定义属性
        if (attrs != null) {
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.VoisePlayingIconView);
            pointerColor = ta.getColor(R.styleable.VoisePlayingIconView_pointer_color, Color.RED);
            pointerNum = ta.getInt(R.styleable.VoisePlayingIconView_pointer_num, 4);//指针的数量，默认为4
            pointerWidth = dp2px(ta.getFloat(R.styleable.VoisePlayingIconView_pointer_width, 5f));//指针的宽度，默认5dp
            pointerSpeed = ta.getInt(R.styleable.VoisePlayingIconView_pointer_speed, 40);
            ta.recycle();
        }
        init();
    }

    private float dp2px(float aFloat) {
        return aFloat * 3;
    }
    ...
    public class MyRunnable implements Runnable {
        ...
        public void run() {
            ...
            for (float i = 0; i < Long.MAX_VALUE; ) {//创建一个死循环，每循环一次i+0.1
                try {
